...
%numsite = numel(site);
%kappa = zeros(numdam, 1); 

% Design assumption (silt: 0.1); coarse(sand) = 1, medium(silt)= 0.1, fine(clay)= 0.046;
%vary kappa with D50 grain size- there are no NaN, every one has a number
kappa = zeros(numdam,1) + 0.046;

% kappa(data.D50_mm_ < 0.002 & data.D50_mm_ > 0) = 0.046;
% kappa(data.D50_mm_ >= 0.002 & data.D50_mm_ < 0.05) = 0.1;
% kappa(data.D50_mm_ >= 0.05) = 1;
% kappa(data.D50_mm_ == 0) = 0.1;
